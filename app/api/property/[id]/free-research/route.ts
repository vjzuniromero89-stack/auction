...
export async function POST(_:Request,{params}:{params:Promise<{id:string}>}){try{
 ...
 for(const type of checks){
   ...
   // court judgments can also expose civil-case leads relevant to liens
   if(['judgments'].includes(type)) src=RESEARCH_SOURCES.filter(s=>s.kind==='judgments');
   ...
   old.data?await db.from('due_diligence_checks').update(row).eq('id',old.data.id):await db.from('due_diligence_checks').insert(row);
   for(const s of src){const ex=await db.from('research_tasks').select('id').eq('property_id',id).eq('task_type',type).eq('provider',s.key).maybeSingle();const task={property_id:id,task_type:type,provider:s.key,status:'manual_review_required',access_cost:'free',execution_mode:s.automated?'automatic':'manual',query_url:s.url,result:{label:s.label,note:s.note,identifiers:{parcel:p.parcel_number,owner:p.owner_name,address:p.address}}};ex.data?await db.from('research_tasks').update(task).eq('id',ex.data.id):await db.from('research_tasks').insert(task);count++}
 }
 await db.from('free_research_runs').update({status:'complete',sources_checked:count,completed_at:new Date().toISOString(),notes:[{mode:'FREE_ONLY'},{rule:'No paid provider calls. Missing access never becomes NOT FOUND.'}]}).eq('id',rr.data.id);
 return NextResponse.json({ok:true,mode:'FREE_ONLY',sources:count,property:{address:p.address,parcel:p.parcel_number,owner:p.owner_name,county:p.county}})
